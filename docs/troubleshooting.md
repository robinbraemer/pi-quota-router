...
Ctrl-C/Escape aborts active usage and provider work. Active reservations renew only while a request remains active and release on completion, provider error, cancellation, or reservation loss. If a process is killed ungracefully, renewal stops and its reservations expire within two minutes.
