...
- User cancellation remains `aborted` and wins deadline races. After the replay boundary, silence timeout, cancellation, and reservation-renewal failure preserve the latest partial content and usage in the terminal event. Reservation-renewal failure terminates the request without account rotation or an account-health failure record. Cleanup for deadlines, completion, heartbeat loss, and cancellation is idempotent.
